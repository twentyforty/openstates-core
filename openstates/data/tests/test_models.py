...
def test_organization_membership_multiple_divisions():
    ...
    d1 = Division.objects.create(
        id="ocd-division/country:aa/place:locality1", name="locality1"
    )
    d2 = Division.objects.create(
        id="ocd-division/country:aa/place:locality2", name="locality2"
    )

    post1 = Post.objects.create(
        label="district rep", role="vip", organization=o, division=d1
    )
    post2 = Post.objects.create(
        label="district rep", role="vip", organization=o, division=d2
    )
    ...
    people = Person.objects.member_of(
        o.id, post="district rep", division_id=d1.id
    ).all()
    assert len(people) == 1
    assert p1 in people

    people = Person.objects.member_of(
        o.id, post="district rep", division_id=d2.id
    ).all()
    assert len(people) == 1
    assert p2 in people
