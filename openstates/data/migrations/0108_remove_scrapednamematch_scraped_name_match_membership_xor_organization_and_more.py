# Generated by Django 4.1.1 on 2024-01-11 01:54

from django.db import migrations, models


class Migration(migrations.Migration):
    dependencies = [
        ("data", "0107_remove_scrapednamematch_match_unique_with_chamber_and_more"),
    ]

    operations = [
        migrations.RemoveConstraint(
            model_name="scrapednamematch",
            name="scraped_name_match_membership_xor_organization",
        ),
        migrations.RemoveConstraint(
            model_name="scrapednamematch",
            name="match_unique_organization_with_chamber",
        ),
        migrations.RemoveConstraint(
            model_name="scrapednamematch",
            name="match_unique_organization_without_chamber",
        ),
        migrations.RemoveConstraint(
            model_name="scrapednamematch",
            name="match_unique_person_with_chamber",
        ),
        migrations.RemoveConstraint(
            model_name="scrapednamematch",
            name="match_unique_person_without_chamber",
        ),
        migrations.RemoveConstraint(
            model_name="scrapednameunresolvedmatch",
            name="unique_with_chamber",
        ),
        migrations.RemoveConstraint(
            model_name="scrapednameunresolvedmatchoption",
            name="unresolved_match_option_membership_xor_organization",
        ),
        migrations.AddConstraint(
            model_name="scrapednamematch",
            constraint=models.UniqueConstraint(
                condition=models.Q(
                    ("matched_chamber_id__isnull", False),
                    ("matched_organization_id__isnull", False),
                ),
                fields=(
                    "legislative_session_id",
                    "matched_chamber_id",
                    "matched_organization_id",
                    "value",
                ),
                name="match_unique_organization_with_chamber",
            ),
        ),
        migrations.AddConstraint(
            model_name="scrapednamematch",
            constraint=models.UniqueConstraint(
                condition=models.Q(
                    ("matched_chamber_id__isnull", True),
                    ("matched_organization_id__isnull", False),
                ),
                fields=("legislative_session_id", "matched_organization_id", "value"),
                name="match_unique_organization_without_chamber",
            ),
        ),
        migrations.AddConstraint(
            model_name="scrapednamematch",
            constraint=models.UniqueConstraint(
                condition=models.Q(
                    ("matched_chamber_id__isnull", False),
                    ("matched_person_id__isnull", False),
                ),
                fields=(
                    "legislative_session_id",
                    "matched_chamber_id",
                    "matched_person_id",
                    "value",
                ),
                name="match_unique_person_with_chamber",
            ),
        ),
        migrations.AddConstraint(
            model_name="scrapednamematch",
            constraint=models.UniqueConstraint(
                condition=models.Q(
                    ("matched_chamber_id__isnull", True),
                    ("matched_person_id__isnull", False),
                ),
                fields=("legislative_session_id", "matched_person_id", "value"),
                name="match_unique_person_without_chamber",
            ),
        ),
        migrations.AddConstraint(
            model_name="scrapednamematch",
            constraint=models.CheckConstraint(
                check=models.Q(
                    models.Q(
                        ("matched_organization_id__isnull", True),
                        ("matched_person_id__isnull", False),
                    ),
                    models.Q(
                        ("matched_organization_id__isnull", False),
                        ("matched_person_id__isnull", True),
                    ),
                    _connector="OR",
                ),
                name="scraped_name_match_membership_xor_organization",
            ),
        ),
        migrations.AddConstraint(
            model_name="scrapednameunresolvedmatch",
            constraint=models.UniqueConstraint(
                condition=models.Q(("chamber_id__isnull", False)),
                fields=("legislative_session", "chamber", "value"),
                name="unique_with_chamber",
            ),
        ),
        migrations.AddConstraint(
            model_name="scrapednameunresolvedmatchoption",
            constraint=models.CheckConstraint(
                check=models.Q(
                    ("person_id__isnull", False),
                    ("organization_id__isnull", False),
                    _connector="OR",
                ),
                name="unresolved_match_option_membership_xor_organization",
            ),
        ),
    ]
